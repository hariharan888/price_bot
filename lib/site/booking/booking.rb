...
module Site
  class Booking < BaseClass
    ...
    def get_nightly_prices(listing, params)
      puts "Fetching prices for: #{listing.name}"

      params => { days: }
      prices = []
      start_date = checkin_date

      while prices.count < days
    ...
    def get_price_request(listing, start_date)
      ...
      set_headers!(request)
      data_raw = get_pricing_request_body(listing, start_date)
      request.body = data_raw

      response = with_cache(url, data_raw) { http.request(request) }
    ...
    def get_listings(params)
      ...
      listings = listings_data.map { |listing| Site::BookingListing.new(listing) }
    ...
    def get_pricing_request_body(listing, start_date)
      search_params_file = File.join(File.dirname(__FILE__), "listing_price_params.json")
